...
C   PRGMMR: KEYSER           ORG: NP22        DATE: 2013-01-17
C
...
C 2009-05-08  Krasowski -- Modified to handle processing of NOAA-19 data
C 2013-01-17  Keyser    -- Modified to handle processing of METOP-1 data
C
C USAGE:
C
C   INPUT FILES:
...
C     SYSTEM:    - SYSTEM
C     LIBRARY:
C       W3LIB    - W3TAGB  W3TAGE  ERREXIT W3FS26  W3MOVDAT W3DOXDAT
...
C  ---------------------------------------
      ...
      integer stdout
      character*500 intxt,outfile,coefile
      character*8  compress,process_Tb,process_Ta,i512
      integer(8)::beginT,endT,recT
      namelist /input/ intxt,outfile,coefile,compress,process_Tb,
     x                 process_Ta,beginT,endT
...
c------201409291400--------------------
      ...
      print *
      print *, 'WELCOME TO BUFR_TRANAMSUA - Version 01/17/2013'
      print *
...
C  --------------------------------------------
      ...
      call amsua(lunin,intxt,luncof,coefile,lunout)
      ...
      SUBROUTINE AMSUA(LUNIN,INTXT,LUNCOF,COEFILE,LUNOUT)
...
C  -----------------
      ...
      character*500 intxt,coefile,rawamsu
...
C  ---------------------------------------
      ...
      write(stdout,*)' '
      write(lunout) nreal-4,mch,(ichan(i),i=1,mch)
...
C---------------add at 201410290900----------------------
 2001 continue

      open(lunintxt,file=intxt)
      read(lunintxt,'(A500)',iostat=stat) rawamsu
      if(stat/=0) goto 1600
      write(*,*) 'File ',rawamsu
C-------------------------------------------------------
...
C  -----------------------------------------------
         if(i512.eq.'YES') then
            do j=1,5
               read (lunin,rec=(nri-1)*5+j+1,err=2001)
     &              (kbuf(i),i=(j-1)*512+1,j*512)
            end do
         else
            do j=1,5
               read (lunin,rec=(nri-1)*5+j,err=2001)
     &              (kbuf(i),i=(j-1)*512+1,j*512)
            end do
         end if
...
C  --------------------------------------
               ...
               bdata_ieee  = jsat_ieee            ! satellite indicator
...
c---------------add 201410290000-------------------------
               recT=INT(bdata(3),8)*INT(1e10,8)+
     x              INT(bdata(4),8)*INT(1e8,8)+
               ...
               if(process_Tb.eq.'YES') then
                  if (ikeepb(i).eq.1) then
                    ...
                    call bufr1b(lubfrb,'NC021023',nreal,mch,bdata,nrepb)
                     write(lunout) (bdata_ieee,bdata(j),j=2,5),sctime1,
     +                (bdata(j),j=9,nreal-2),
     +                (bdata(j),j=nreal+1,ntot,nperchan) ! ieee write
